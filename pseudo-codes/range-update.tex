\begin{algorithm}
    \caption{Range Update on Segment Tree }
    \begin{algorithmic}
        \Procedure{Push}{seg, lazy, index, l, r}
            \State $size = r - l$
            \If{$lazy[index] \neq 0$}
                \State $seg[index*2] \gets seg[index*2] + lazy[index] * \left( \frac{size}{2} \right)$
                \State $lazy[index*2] \gets lazy[index*2] + lazy[index]$
                \State $seg[index*2+1] \gets seg[index*2+1] + lazy[index] * \left( \frac{size}{2} \right)$
                \State $lazy[index*2+1] \gets lazy[index*2+1] + lazy[index]$
                \State $lazy[index] \gets 0$
            \EndIf
        \EndProcedure

        \\

        \Procedure{RangeUpdate}{seg, lazy index, l, r, a, b, value} \Comment{Value: value we wanted to add to each element in array}
            \State $size = r - l$
            \If{$b \leq l$ \textbf{or} $r \leq a$}
                \State \Return
            \ElsIf{$a \leq l$ \textbf{and} $r \leq b$}
                \State $seg[index] \gets {seg[index] + value*size}$
                \State $lazy[index] \gets {lazy[index] + value}$
                \State \Return
            \Else
                \State \Call{Push}{seg, lazy, index, l, r}
                \State $mid \gets \left\lfloor \frac{l + r}{2} \right\rfloor$
                \State $left \gets$ \Call{QueryUpdate}{seg, $2 \cdot index$, $l$, $mid$, $a$, $b$, $value$}
                \State $right \gets$ \Call{QueryUpdate}{seg, $2 \cdot index + 1$, $mid$, $r$, $a$, $b$, $value$}
                \State $seg[index] \gets {seg[index*2] + seg[index*2+1] + lazy[index]*size}$ \Comment{Update current node after lazy propagation}
            \EndIf
        \EndProcedure
    \end{algorithmic}
\end{algorithm}